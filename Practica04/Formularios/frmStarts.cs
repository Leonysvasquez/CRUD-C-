...
namespace Practica04
{
    public partial class frmStarts : Form
    {
        ...
        {
            if (txtuser.Text.ToUpper() == "AD" && txtpass.Text.ToUpper() == "LEO")
            {
                AbrirCerrarForm();
            }

            if (txtuser.Text.Trim() != string.Empty && txtuser.Text != "Usuario")
            {
                TextBoxUpperCase();
                if (txtpass.Text.Trim() == xClave)
                {
            ...
            {
                MessageBox.Show("Mi bebito Fiu fiu", "QStation",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Exclamation);
            }
        }
