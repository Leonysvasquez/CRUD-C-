...
{
    public partial class VENTLocalidad : Form
    {
        public string varf1;
        public string varf2;
        ...
        public VENTLocalidad()
        {
            InitializeComponent();
        }

        private void VENTProvincia_Load(object sender, EventArgs e)
        {

            this.KeyPreview = true;     // puedo presionar la tecla escape
            this.Text = HMenu.cia + "   Consulta Provincia";

            this.dgv.EnableHeadersVisualStyles = false;
            this.dgv.ColumnHeadersVisible = true;
            this.dgv.RowHeadersVisible = false;

            this.dgv.Columns.Add("Col00", "ID Localidad");
            this.dgv.Columns.Add("Col01", "Nombre de Fabrica");
            this.dgv.Columns.Add("Col01", "Localidad");
        ...
        private void VENTProvincia_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void BuscarDatos()
        {
            ...
            SqlCommand sqCmd = new SqlCommand("SELECT IDLocalidad,NombreLocalidad "
                                              + "  FROM LOCALIDAD "
                                              + txtBuscar.Text , conexion);
            ...
            try
            {
                ...
                {  // no es fin de archivo true false
                    ...
                    dgv[0, xRows].Value = Convert.ToString(recordset["IDLocalidad"]);     // escribe en la grilla, en la celda 0 y en la fila (x), el contenido del campo IDDEPARTAMENTO     de la tabla DEPARTAMENTO
                    dgv[1, xRows].Value = Convert.ToString(recordset["NombreLocalidad"]); // escribe en la grilla, en la celda 1 y en la fila (x), el contenido del campo NOMBREDEPARTAMENTO de la tabla DEPARTAMENTO
